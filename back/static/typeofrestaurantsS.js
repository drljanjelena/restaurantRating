window.addEventListener("load", () => {
    ...
    function updateTaskList(tasks){
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch("http://localhost:4000/typeofrestaurants/obrisi/"+this.parentNode.parentNode.dataset.taskid,
         ...
         for(i=0; i < btns_edits.length; i++){
             btns_edits[i].addEventListener("click", function(){
                 ...
                 document.getElementById("save").addEventListener("click", function(){
                     ...
                     fetch('http://localhost:4000/typeofrestaurants/edit/'+ id,{
                         method:"PUT",
                         headers: {
    ...
    document.getElementById("btn_dodaj").addEventListener("click", function(){
        let noviRestoran = {
            name: document.getElementById("novi_type_type").value,
        };
        nt = JSON.stringify(noviRestoran);
        fetch("http://localhost:4000/typeofrestaurants/dodaj",
                {
                    method:"POST",
                    headers: {
                      'Accept': 'application/json',
                ...
                document.getElementById("novi_type_type").value = '';
    });
