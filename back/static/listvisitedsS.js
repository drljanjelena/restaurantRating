window.addEventListener("load", () => {
    ...
    function updateTaskList(tasks){
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch("http://localhost:4000/listvisited/obrisi/"+this.parentNode.parentNode.dataset.taskid,
        ...
        for(i=0; i < btns_edits.length; i++){
            btns_edits[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    ...
                    let novi = {
                        dateOfVisit: document.getElementById("date").value
                    };
                    nt = JSON.stringify(novi);
                    fetch('http://localhost:4000/listvisited/edit/'+ id,{
                        method:"PUT",
                        headers: {
    ...
    document.getElementById("btn_dodaj").addEventListener("click", function(){
        let noviRestoran = {
            userID: document.getElementById("novi_lista_user_id").value,
            dateOfVisit: document.getElementById("novi_task_datum").value,
        };
        nt = JSON.stringify(noviRestoran);
        fetch("http://localhost:4000/listvisited/dodaj",
                {
                    method:"POST",
                    headers: {
                      'Accept': 'application/json',
                ...
                document.getElementById("novi_lista_user_id").value = '';
    ...
    .then(response => response.json())
    .then(data => {
    var combobox =document.getElementById("novi_lista_user_id");
